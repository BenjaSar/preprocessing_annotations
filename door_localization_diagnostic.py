"""T-KG1b: diagnose WHY CubiCasa5K scores 0 TP@0.5 for doors.
...
from bbox_metrics import BBox, iou
from config import (
    EvalConfig,
...
def _diagnose(
    ...
    for gt_image in gt_images:
        preds = detect_with_cubicasa(detector, gt_image.image_path)
...
def _build_detector() -> CubiCasa5KDetector:
    """Load the CubiCasa5K detector once."""
...
def _parse_args() -> argparse.Namespace:
    """Parse CLI args: category, dataset selection, split, sample cap."""
    parser = argparse.ArgumentParser(
        description="T-KG1b: door localization failure diagnostic"
    )
...
def main() -> None:
    ...
    results = {
        name: _diagnose(detector, _GT_LOADERS[name](args), args.category)
