...
MANDATORY_CLASSES: Dict[str, List[str]] = {
    ...
    "MULTIPURPOSE ROOM": [
        ...
        # Shared program/amenity spaces, per explicit taxonomy directive.
        # Previously absent here, so each fell through to the STORAGE ROOM
        # fallback in normalize_to_mandatory().
        "SOCIAL SERVICE", "SOCIAL SERVICES",
        "STUDENT SERVICES",
        "RECREATION", "RECREATION ROOM",
        "CHILDREN'S PLAY ROOM", "CHILDRENS PLAY ROOM", "PLAY ROOM",
    ],
    ...
    "OTHER": [
        ...
        # Same amenity family as YOGA/MEDITATION ROOM above; the slash form did
        # not resolve the space-separated label ("YOGA ROOM" is neither a
        # substring of it nor vice-versa), so it fell back to STORAGE ROOM.
        "YOGA ROOM", "YOGA",
    ],
}

# ---------------------------------------------------------------------------
# Generalized surface rules
# ---------------------------------------------------------------------------
# Label forms that a fixed variant list cannot express because they carry a
# variable part (a room number). Consulted by normalize_to_mandatory() AFTER
# the exact surface-form lookup and BEFORE the substring fallbacks, so a
# precise generic label is not captured by an unrelated variant through the
# bidirectional substring test — measured: "ROOM 2" matched the extended
# variant "BEDROOM 2" via `raw_upper in variant` and resolved to
# RESIDENTIAL UNIT.
#
# Deliberately anchored: an unanchored rule would reclassify any label merely
# containing a numbered-room fragment.
_REGEX_SURFACE_TO_MANDATORY: List[tuple] = [
    (re.compile(r"^ROOM\s*\d+$", re.IGNORECASE), "OTHER"),
]
...
def normalize_to_mandatory(raw: str) -> str:
    ...
    # 3a. Generalized surface rules (numbered generic rooms). Placed after the
    # exact surface lookup so a specific variant always wins, and before the
    # substring fallbacks so the variable part is not mis-captured by an
    # unrelated variant. See _REGEX_SURFACE_TO_MANDATORY.
    for _pattern, _mandatory_type in _REGEX_SURFACE_TO_MANDATORY:
        if _pattern.match(raw_stripped):
            logger.debug(f"Regex surface rule: '{raw}' → '{_mandatory_type}'")
            return _mandatory_type
