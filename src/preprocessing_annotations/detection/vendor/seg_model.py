...
class Residual(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv1 = nn.Conv2d(numIn, numOut // 2, bias=True, kernel_size=1)
        self.bn1 = nn.BatchNorm2d(numOut // 2)
        self.conv2 = nn.Conv2d(numOut // 2, numOut // 2, bias=True, kernel_size=3, stride=1, padding=1)
        self.bn2 = nn.BatchNorm2d(numOut // 2)
        self.conv3 = nn.Conv2d(numOut // 2, numOut, bias=True, kernel_size=1)
        if numIn != numOut:
            self.conv4 = nn.Conv2d(numIn, numOut, bias=True, kernel_size=1)
